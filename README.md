...
End-to-end flow: DeepStream runs YOLO, publishes RTSP, and sends person counts directly to MQTT (`deepstream/person_count`). MediaMTX pulls RTSP and serves WHEP/WebRTC to the web UI. The web UI shows the stream and live counts. An LED notifier subscribes to MQTT and blinks a GPIO LED when person_count ≥ threshold. All of these can be started/stopped from the web “Start Pipeline” button.
